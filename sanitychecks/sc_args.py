...
def add_args(parser):
    ...
    parser.add_argument(
        "--n_points", type=int, default=15000, help="number of points in each point cloud"
    )
    # parser.add_argument('--delta', type=float, default=0.25, help='clamping value')

    # Network architecture and loss
    parser.add_argument(
        "--decoder_hidden_dim", type=int, default=128, help="length of decoder hidden dim"
    )
    parser.add_argument(
        "--encoder_hidden_dim", type=int, default=128, help="length of encoder hidden dim"
    )
    parser.add_argument(
        "--decoder_n_hidden_layers", type=int, default=4, help="number of layers in the decoder"
    )
    parser.add_argument("--nl", type=str, default="sine", help="type of non linearity sine | relu")
    parser.add_argument(
        "--latent_size", type=int, default=0, help="number of elements in the latent vector"
    )
    parser.add_argument(
        "--sphere_init_params", nargs="+", type=float, default=[1.6, 1.0], help="radius and scaling"
    )
    parser.add_argument("--neuron_type", type=str, default="quadratic", help="type of neuron")

    parser.add_argument(
        "--normalize_normal_loss", type=int, default=False, help="normal loss normalization flag"
    )
    parser.add_argument(
        "--unsigned_n", type=int, default=True, help="flag for unsigned normal loss"
    )
    parser.add_argument(
        "--unsigned_d", type=int, default=False, help="flag for unsigned distance loss"
    )
    ...
    parser.add_argument(
        "--export_vis", type=int, default=False, help="export levelset visualization while training"
    )

    parser.add_argument(
        "--loss_type",
        type=str,
        default="siren_wo_n_w_div",
        help="loss type to use: siren | siren_wo_n | siren_wo_n_w_div | siren_wo_n_w_div_w_reg",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="exp",
        help="type of inter los exp | unsigned_diff | signed_diff",
    )
    parser.add_argument(
        "--sampler_prob",
        type=str,
...
def get_test_args():
    ...
    parser.add_argument(
        "--noise_type", type=str, default="gaussian", help="type of noise to inject the data"
    )
    parser.add_argument(
        "--normal_vis", type=int, default=False, help="flag to visualize normals in plot"
    )
    ...
    (
        ...
        test_opt.n_loss_type,
        test_opt.normalize_normal_loss,
        test_opt.unsigned_n,
        test_opt.unsigned_d,
        test_opt.loss_type,
        test_opt.model_dirpath,
        test_opt.inter_loss_type,
        test_opt.shape_type,
        test_opt.div_decay,
        test_opt.div_type,
        test_opt.div_clamp,
        ...
        train_opt.n_loss_type,
        train_opt.normalize_normal_loss,
        train_opt.unsigned_n,
        train_opt.unsigned_d,
        train_opt.loss_type,
        train_opt.model_dirpath,
        train_opt.inter_loss_type,
        train_opt.shape_type,
        train_opt.div_decay,
        train_opt.div_type,
        train_opt.div_clamp,
