...
parser.add_argument(
    "--output_dir",
    type=str,
    default="./log/initialization_vis/",
...
parser.add_argument(
    ...
    type=bool,
    default=True,
    help="toggle on to plot second derivative images",
)
...
parser.add_argument(
    ...
    type=str,
    default="mfgi",
    help="specify type of initialization mfgi | geometric_relu | geometric_sine | siren",
)
...
parser.add_argument(
    "--decoder_hidden_dim", type=int, default=128, help="number units in the decoder"
)
...
test_dataloader = torch.utils.data.DataLoader(
    test_set, batch_size=1, shuffle=False, num_workers=2, pin_memory=True
)
# get model
...
fig = vis.plot_sdf_surface(
    x_grid, y_grid, z_grid, show=False, show_ax=True, title_txt="sdf initialization surface"
)
fig.write_html(
    os.path.join(
        args.output_dir,
        "sdf_surface" + str(args.decoder_hidden_dim) + "_" + args.init_type + ".html",
    )
)
im = Image.fromarray(contour_img)
im.save(
    os.path.join(
        args.output_dir, "sdf_" + str(args.decoder_hidden_dim) + "_" + args.init_type + "_init.png"
    )
)
...
im.save(
    os.path.join(
        args.output_dir,
        "eikonal_" + str(args.decoder_hidden_dim) + "_" + args.init_type + "_init.png",
    )
)
...
im.save(
    os.path.join(
        args.output_dir,
        "zdiff_" + str(args.decoder_hidden_dim) + "_" + args.init_type + "_init.png",
    )
)
if args.plot_second_derivs:
    ...
    im.save(
        os.path.join(
            args.output_dir,
            "div_" + str(args.decoder_hidden_dim) + "_" + args.init_type + "_init.png",
        )
    )
