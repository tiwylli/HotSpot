...
IDENTIFIER='PHASE-boundary-compare'                       # change to your desired identifier
...
for SHAPE_TYPE in 'square'; do
    for coef in 0.1 0.2 0.5 1.0 2.0 5.0 10.0 15.0 20.0; do
        cp -r scripts/$THIS_FILE $LOG_DIR
        cp -r $CONFIG_DIR $LOG_DIR
        echo $SHAPE_TYPE
        python3 train/train.py --config $CONFIG_DIR --log_dir $LOG_DIR/$IDENTIFIER$TIMESTAMP/$SHAPE_TYPE-$coef --model_dir $MODEL_DIR --shape_type $SHAPE_TYPE --loss_weights $coef 0 0 1 0 0 0 1
    done
done
