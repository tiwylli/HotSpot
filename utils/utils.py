...
def log_images(
    writer, iteration, contour_img, curl_img, eikonal_img, div_image, z_diff_img, example_idx
):
    # helper function to log images to tensorboardx writer
    writer.add_image(
        "implicit_function/" + str(example_idx), contour_img.transpose(2, 0, 1), iteration
    )
...
def compute_props(decoder, latent, z_gt, device):
    ...
    if latent is not None:
        grid_points_latent = torch.cat(
            [latent.expand(grid_points.shape[0], -1), grid_points], dim=1
        )
    else:
    ...
    z_diff = np.abs(np.abs(np.reshape(z_np, [res, res])) - np.abs(z_gt)).reshape(
        x.shape[0], x.shape[0]
    )
...
def get_2d_grid_uniform(resolution=100, range=1.2, device=None):
    ...
    grid_points = get_cuda_ifavailable(
        torch.tensor(np.vstack([xx.ravel(), yy.ravel()]).T, dtype=torch.float), device=device
    )
...
def get_3d_grid(
    ...
    if shortest_axis == 0:
        ...
        y = np.arange(
            bbox[1, 0] - eps,
            bbox[1, 1] + length / (x.shape[0] - 1) + eps,
            length / (x.shape[0] - 1),
        )
        z = np.arange(
            bbox[2, 0] - eps,
            bbox[2, 1] + length / (x.shape[0] - 1) + eps,
            length / (x.shape[0] - 1),
        )
        ...
        x = np.arange(
            bbox[0, 0] - eps,
            bbox[0, 1] + length / (y.shape[0] - 1) + eps,
            length / (y.shape[0] - 1),
        )
        z = np.arange(
            bbox[2, 0] - eps,
            bbox[2, 1] + length / (y.shape[0] - 1) + eps,
            length / (y.shape[0] - 1),
        )
        ...
        x = np.arange(
            bbox[0, 0] - eps,
            bbox[0, 1] + length / (z.shape[0] - 1) + eps,
            length / (z.shape[0] - 1),
        )
        y = np.arange(
            bbox[1, 0] - eps,
            bbox[1, 1] + length / (z.shape[0] - 1) + eps,
            length / (z.shape[0] - 1),
        )
    ...
    grid_points = torch.tensor(
        np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T, dtype=torch.float16
    )
    return {
...
def recon_metrics(
    ...
    for alpha in alphas:
        pod12.append(
            ((one_distances < alpha * scale).sum() + (two_distances < alpha * scale).sum())
            / (one_distances.shape[0] + two_distances.shape[0])
        )
    cdp12 = np.percentile(
        np.concatenate([one_distances, two_distances]), percentiles
    )  # compute chamfer distance percentiles cdp

    nn1_dist, local_idx2 = pc1_kd_tree.query(pc1, max(k), n_jobs=-1)
    nn1_dist_2pc2 = two_distances[local_idx2]
    malcv = [
        (nn1_dist_2pc2[:, :k0] / nn1_dist.mean(axis=1, keepdims=True)).var(axis=1).mean()
        for k0 in k
    ]
    ...
    if return_all:
        return (
            chamfer_distance,
            hausdorff_distance,
            (cd12, cd21, cdmed12, cdmed21, hd12, hd21),
            (pod1, pod2, pod12),
            (cdp1.tolist(), cdp2.tolist(), cdp12.tolist()),
            malcv,
            (one_distances, two_distances),
        )
...
def implicit2mesh(
    ...
    for point in tqdm(torch.split(pnts, 100000, dim=0)):
        ...
        if not latent is None:
            point = torch.cat(
                [
                    point,
                    latent.unsqueeze(0).repeat(point.shape[0], 1),
                ],
                dim=1,
            )
    ...
    if get_mesh:
        mesh = trimesh.Trimesh(
            verts, faces, vertex_normals=normals, vertex_colors=values, validate=True
        )
    trace = None
    ...
    return {"mesh_trace": trace, "mesh_obj": mesh}
...
def convert_xyz_to_ply_with_noise(file_path, noise=None):
    ...
    if noise is None:
        ...
        for std in noise:
            ...
            var = std * std
            cov_mat = bbox_scale * np.array([[var, 0.0, 0.0], [0.0, var, 0.0], [0.0, 0.0, var]])
            noise = np.random.multivariate_normal(
                [0.0, 0.0, 0.0], cov_mat, size=points.shape[0], check_valid="warn", tol=1e-8
            )
...
def get_grid(grid_range, grid_res, in_dim=2):
    # Return grid points with shape (grid_res, grid_res, in_dim)
    x, y = np.linspace(-grid_range, grid_range, grid_res), np.linspace(
        -grid_range, grid_range, grid_res
    )
    xx, yy = np.meshgrid(x, y)
    xx, yy = xx.ravel(), yy.ravel()
    grid_points = np.stack([xx, yy], axis=-1)
    if in_dim == 3:
        z = np.zeros((grid_res, grid_res, 1))
