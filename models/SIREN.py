...
class FCBlock(MetaModule):
    ...
    def __init__(
        ...
        if init_type == "siren":
            ...
            self.net[-2].apply(geom_relu_last_but_one_layers_init)
...
def second_last_layer_geom_sine_init(m):
    with torch.no_grad():
        if hasattr(m, "weight"):
            ...
            m.weight.data = 0.5 * np.pi * torch.eye(num_output) + 0.001 * torch.randn(
...
def geom_relu_last_but_one_layers_init(m):
    radius_init = 0.0
    with torch.no_grad():
        if hasattr(m, "weight"):
            num_input = m.weight.size(-1)
